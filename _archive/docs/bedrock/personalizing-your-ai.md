...
**Who this is for:** Someone who already uses an AI tool and wants it to stop feeling like they are re-briefing a stranger every session.

**What you will leave with:** You have set up persistent instructions in your tool of choice so it already knows who you are, how you communicate, and what to never do — before you type a single word.
...
- Sensitive information, PII, or anything covered in the Data Handling module — custom instructions go to the platform's servers on every request
- Classified or controlled material of any kind
- Instructions that contradict the platform's own guidelines
...
??? note "Instructor Note — Security Reminder"
    Remind students of the Data Handling module before they write custom instructions. Everything in a custom instruction goes to the platform's servers on every request. Nothing sensitive, nothing controlled, nothing above the system's authorization ceiling.
...
In an agentic context, a well-defined persona means you do not re-explain the agent's role and constraints every time you start a session. The persona carries it.
